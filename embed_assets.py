...
# Only tiny assets are inlined. Anything larger streams from ASSET_DIR/ like the
# landmark sprites already do, because inlining big art is a false economy: a
# base64 data URI is ~33% bigger than the file, it cannot be cached separately,
# and it has to finish downloading before the browser can parse the script and
# start the game. Inlining 1.2 MB of cloud art used to make index.html 1.8 MB,
# so nobody could play until all of it had arrived. The clouds, the ground and
# the noise overlay are all things you can afford to see a moment late.
...
for filename in sorted(os.listdir(ASSET_DIR)):
    ...
    if filename not in referenced:
        ...
        continue
    if os.path.getsize(filepath) > MAX_INLINE_BYTES:
        # Big enough that streaming beats blocking the parse (see MAX_INLINE_BYTES).
        streamed.append(filename)
        continue
...
print(f'Streaming {len(streamed)} files over {MAX_INLINE_BYTES} bytes: '
      f"{', '.join(sorted(streamed))}")
...
# ASSET_BASE_PATH survives whenever a file it names is streamed rather than inlined
# (the clouds, now). Deleting the const in that case would ship a ReferenceError and
# CloudSystem would die at boot, so the const is only removed once nothing needs it.
leftover = sorted(set(re.findall(pattern, html)))
if leftover:
    missing = [n for n in leftover if n not in on_disk]
    if missing:
        print("ERROR: ASSET_BASE_PATH names files that are not on disk:")
        for name in missing:
            print(f"  - {name}")
        sys.exit(1)
    print(f'Kept ASSET_BASE_PATH for {len(leftover)} streamed file(s)')
else:
...
banner = (
    ...
    f"  ground and the noise overlay all load from '{ASSET_DIR}/' at runtime, so this\n"
