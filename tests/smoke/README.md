...
The unit suite (`node --test tests/*.test.mjs`) covers the game's **pure logic** but
...
- the grip-slider default (`gripMultiplier` = 1.0 at slider `20`/"20%"),
...
# (tests/smoke/node_modules + manifests are gitignored)
npm --prefix tests/smoke i -D playwright-core
npx --prefix tests/smoke playwright install chromium   # or set SMOKE_CHROMIUM instead
...
```sh
SMOKE_CHROMIUM="/path/to/Chromium" node tests/smoke/smoke.mjs
```
...
`tests/*.test.mjs` glob), so it never runs — or fails — in the dependency-free path.
