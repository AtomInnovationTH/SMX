...
set -euo pipefail
cd "$(dirname "$0")/.."
echo "== 1/4 unit tests =="         ; node --test tests/*.test.mjs
...
echo "== 3/4 asset references =="   ; python3 tools/check_refs.py
if [ "${SKIP_SMOKE:-0}" = "1" ]; then echo "== 4/4 smoke: skipped =="
else echo "== 4/4 browser smoke =="  ; node tests/smoke/smoke.mjs
fi
echo "GATE PASS"
