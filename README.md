...
> A ground station beams longitudinal (compression) waves up a conductive graphene/CNT space-elevator seed tether. An up-climber rides them contactlessly: its electro-permanent-magnet hands pulse to induce eddy currents in the moving tether. Its onboard controller engages the magnet stack exactly when the film is outrunning it, so every cycle nets upward momentum; the switching energy comes back out of the mechanical power the climber extracts, and when it stops covering that draw the climber has to let go. **Nothing ever touches** — momentum crosses an air gap. It carries cargo (tether material) toward the Kármán Line. Down-climbers — which would brake regeneratively on descent and reinforce the tether — are the next chapter.

**Why an electro-permanent magnet, specifically?** An electromagnet burns power continuously to hold a field; a permanent magnet cannot be switched off. An EPM is the only option that is *on for free and off for free*, paying a few joules per transition. **That zero static power is the enabling technology** — without it, a climber would spend more energy holding on than climbing. The real hardware this is modelled on is [Zubax FluxGrip FG40](https://fluxgrip.zubax.com/).
